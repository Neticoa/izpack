...
public class IzPackNewMojoTest extends AbstractMojoTestCase {
	...
	public void testExecute() throws Exception {
		...
		assertThat((ZipFile) jar,
				ZipMatcher.isZipMatching(
						IsCollectionContaining.hasItems("com/izforge/izpack/core/container/AbstractContainer.class",
	...
	public void testFixIZPACK_903() throws Exception {
		...
		// assertEquals(classifier, getVariableValueFromObject(mojo, ""));

		// Verify the generated file exists.
		assertThat(file.exists(), Is.is(true));
		assertEquals(project.getArtifact().getFile().getCanonicalPath(), file.getCanonicalPath()); 
	}
	...
	public void testFixIZPACK_1400() throws Exception {
		...
		File testPom = new File(
				Thread.currentThread().getContextClassLoader().getResource("pom-izpack-1400.xml").toURI());
		IzPackNewMojo mojo = (IzPackNewMojo) lookupMojo("izpack", testPom);
		assertThat(mojo, IsNull.notNullValue());
		initIzpack5Mojo(mojo);
		...
		// project.Properties do not reflect the user properties, so property1 reflects
		// pom.xml
		...
		// The IzPackMaven plugin should honor the user property set with
		// "-Dproperty1=value1"
